...
const convert = /* istanbul ignore next */ ({ value, EMPTY_UI_VALUE, direction, converter, edge, i18n }) => {
...
const getRange = /* istanbul ignore next */ ({ value, EMPTY_UI_VALUE, direction, converter, i18n }) => ['from', 'to'].
  reduce(
    (rez, edge) => ({
      ...rez,
...
const getConverter = /* istanbul ignore next */ ({ EMPTY_UI_VALUE, converter }) => ['format', 'parse'].reduce(
  (rez, direction) => ({
    ...rez,
...
export const buildRangeFieldType = /* istanbul ignore next */ (baseFieldType, baseUiTypes) => ({
  isValid(value) {
